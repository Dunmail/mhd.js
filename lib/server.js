...
function start(config)
{
  ...
  svc.get("/net.ihe/DocumentDossier/", function(req, res){res.send(400, "Bad Request");});
  ...
  svc.get("/net.ihe/Document/", function(req, res){res.send(400, "Bad Request");});
  svc.get("/net.ihe/Document/:entryUuid/", getDocument.validateRequest, getDocument.handle);
  svc.get("*", function(req, res){res.send(403, "Request not supported")});
