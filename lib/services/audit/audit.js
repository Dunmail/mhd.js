...
// This opens up the writeable stream to `output`
    var writeStream = fs.createWriteStream('./logs/activity.log', {'flags': 'a'});
    writeStream.write(atnaAuditMessage_Server());
    writeStream.write('\n');
...
function atnaAuditMessageTemplate(){
    ...
    tmp.push('<EventID EventDateTime=":date" EventOutcomeIndicator=":outcome">');
    tmp.push(':method :url');
    tmp.push('</EventID>');
    tmp.push('</EventIdentification>');
...
function atnaAuditMessage_Server(){
    var tmp = [];
    tmp.push('<AuditMessage>');
    tmp.push('<EventIdentification>');
    tmp.push('<EventID EventDateTime="' + new Date().toUTCString() + '" EventOutcomeIndicator="0">');
