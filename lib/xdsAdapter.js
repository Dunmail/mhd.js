...
var classificationSchemeformatCode = classificationSchemeUnknown;
var classificationSchemetypeCode = classificationSchemeUnknown;
...
function getSlotValue(xmlElement, name) {
    return xmlElement.get("//rim:Slot[@name='" + name + "']/rim:ValueList/rim:Value", namespaces).text();
}
...
function getAuthorValue(xmlElement) {
    ...
    if (classificationNode) {
        author["authorPerson"] = classificationNode.get("rim:Slot[@name='authorPerson']/rim:ValueList/rim:Value", namespaces).text();
    }
    return author;
}
